...
# (Co)Trignometric functions
csc(x::Interval) = divide_positive_constant(1.0,sin(x))
sec(x::Interval) = divide_positive_constant(1.0,cos(x))
cot(x::Interval) = divide_positive_constant(1.0,tan(x))
acsc(x::Interval) = asin(divide_positive_constant(1.0,x))
asec(x::Interval) = acos(divide_positive_constant(1.0,x))
acot(x::Interval{T}) where T = half_pi(T) - atan(x)

# Scaled Trignometric functions
